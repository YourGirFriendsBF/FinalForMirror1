...
from bot import bot, Interval, INDEX_URL, BUTTON_FOUR_NAME, BUTTON_FOUR_URL, BUTTON_FIVE_NAME, BUTTON_FIVE_URL, \
                ...
                download_dict, download_dict_lock, SOURCE_LINK, TG_SPLIT_SIZE, LOGGER, MEGA_KEY, DB_URI, INCOMPLETE_TASK_NOTIFIER, TITLE_NAME, LEECH_LOG, LINK_LOGS, CMD_INDEX
...
def _mirror(bot, message, isZip=False, extract=False, isQbit=False, isLeech=False, pswd=None, multi=0, select=False, seed=False):
    ...
    if not is_url(link) and not is_magnet(link) and not ospath.exists(link):
        help_msg = f"<b>Kindly Send Link With the Proper Command else Check help{CMD_INDEX} command!</b>"
